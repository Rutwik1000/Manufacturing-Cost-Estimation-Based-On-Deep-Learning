...
def my_temp(c,df,units):
    ...
    c.drawString(0*inch,8.3*inch,df.columns[0])#index
    c.drawString(0*inch,8.1*inch,units[0])
    
    c.drawString(0.5*inch,8.3*inch,df.columns[1])#Operation
    c.drawString(0.5*inch,8.1*inch,units[1])
    
    c.drawString(2*inch,8.3*inch,df.columns[2])#Cutting Speed
    c.drawString(2*inch,8.1*inch,units[2])
    
    c.drawString(3.7*inch,8.3*inch,df.columns[3])#Rate
    c.drawString(3.6*inch,8.1*inch,units[3])
    
    c.drawString(5*inch,8.3*inch,df.columns[4])#Time
    c.drawString(5*inch,8.1*inch,units[4])
    
    c.drawString(6.1*inch,8.3*inch,df.columns[5])#cost
    c.drawString(6.1*inch,8.1*inch,units[5])
    
    c.setStrokeColorCMYK(0,0,0,1) # vertical line colour 
    c.line(1.8*inch,8.3*inch,1.8*inch,2.7*inch)# first vertical line
    c.line(3.4*inch,8.3*inch,3.4*inch,2.7*inch)# second vertical line
    c.line(4.7*inch,8.3*inch,4.7*inch,2.7*inch)# third vertical line
    c.line(5.8*inch,8.3*inch,5.8*inch,2.7*inch)# fourth vertical line
...
def get_pdf(df,units,filename,shapetypes,machtypes):
    print(filename)
    my_path=r'Staging Area/'+str(filename)+'.pdf' 
    print(my_path)
    discount_rate=10 # 10% discount 
    tax_rate=12 # tax rate  in percentage 

    c = canvas.Canvas(my_path,pagesize=letter)
    c=my_temp(c,df,units) # run the template
    ...
    for index, items in df.iterrows():
        c.drawString(0*inch,line_y*inch,str(int(items[0]))) # Index
        c.drawRightString(1.5*inch,line_y*inch,str(machtypes[int(items[1])])) # Operation
        c.drawRightString(2.8*inch,line_y*inch,str(items[2])) #Cutting Speed
        c.drawRightString(4.2*inch,line_y*inch,str(items[3])) #  Rate
        c.drawRightString(5.5*inch,line_y*inch,str(items[4]))#Time
        c.drawRightString(6.6*inch,line_y*inch,str(items[5]))#Cost
